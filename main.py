...
모드:
  python main.py           # --test (기본): 더미 데이터로 Discord 스레드 흐름 검증
  python main.py --live    # 실제 수집 + 스케줄러 실행 (Discord 봇 토큰 필요)
...
# ---------------------------------------------------------------------------
# --test 모드: 더미 데이터로 3가지 Discord 흐름 검증
# ---------------------------------------------------------------------------

async def run_test(qianyu: QianyuBot, perlica: PerlicaBot):
    await asyncio.gather(qianyu.wait_ready(), perlica.wait_ready())
    print("\n[test] 두 봇 모두 온라인. 더미 시나리오 시작.")
    print(f"  루머 → Level {RUMOR_NEWS.alert_level} | "
          f"속보 → Level {BREAKING_NEWS.alert_level}\n")

    scheduler = AIFieldScheduler(qianyu, perlica)
    perlica.set_scheduler(scheduler)

    # 시나리오 A: 루머 → 스레드에 Perlica 검증
    print("[A] Qianyu 루머 포스팅 + Perlica 검증 스레드")
    await scheduler._post_immediate(RUMOR_NEWS)
    await asyncio.sleep(2)

    # 시나리오 B: 공식 속보 → 스레드에 Qianyu 반응
    print("[B] Perlica 속보 포스팅 + Qianyu 커뮤니티 반응 스레드")
    await scheduler._post_immediate(BREAKING_NEWS)
    await asyncio.sleep(2)

    # 시나리오 C: 즉시 브리핑 강제 발송
    print("[C] Perlica 브리핑 발송")
...
async def run_live(briefing_now: bool = False):
    ...
    qianyu_task = asyncio.create_task(qianyu.start(config.QIANYU_BOT_TOKEN))
    perlica_task = asyncio.create_task(perlica.start(config.PERLICA_BOT_TOKEN))

    await asyncio.gather(qianyu.wait_ready(), perlica.wait_ready())
    print("[live] 두 봇 모두 온라인. 스케줄러 시작.")
    ...
    qianyu_task.cancel()
    perlica_task.cancel()
    await asyncio.gather(qianyu_task, perlica_task, return_exceptions=True)

# ---------------------------------------------------------------------------
# --test 모드 진입점 (봇 시작 + 테스트 + 종료)
# ---------------------------------------------------------------------------
