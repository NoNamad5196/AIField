...
def rumor_user_prompt(item: NewsItem) -> str:
    ...
    lines = [
        ...
        "- **굵게** 등 기본 텍스트 강조만 허용",
        "- 메시지 구성: ① 첫 줄 진천우 인트로 ② **제목 굵게** ③ 내용 요약 3~5줄 ④ 마지막 한 줄에 수치+한줄평",
        "- 수치는 맨 마지막 한 줄에만 (예: `신뢰 3 | 긴급 4 | 실용 3`), 본문보다 눈에 띄면 안 됨",
        "- 핵심 내용을 반드시 한국어로 3~5줄 요약 (영어면 번역 후 풀어서 설명, 원문 영어 그대로 복사 금지)",
        "- 진천우 말투: 밝고 신난 반말, 관리자에게 친근하게",
        "- 루머라면 '아직 공식은 아니야' 한 줄 포함",
        "- 요약에 '[YouTube: 제목]' 형태가 있으면: 영상 제목을 보고 대충 어떤 내용일지 진천우 시각으로 1~2줄 코멘트 추가",
        "  (예: '제목 보니까 Codex로 AI 에이전트 작업 자동화하는 내용인 것 같아. 개발 자동화 관심 있으면 볼 만해.')",
...
def community_reaction_user_prompt(item: NewsItem) -> str:
    ...
    return "\n".join([
        ...
        "- Discord Markdown 사용",
        "- 4~7줄 이내",
        "- 핵심 내용을 반드시 한국어로 3~5줄 요약해서 포함 (영어 내용이면 번역 후 풀어서 설명, 원문 영어 그대로 복사 금지)",
        "- 진천우 말투: 밝고 친근한 반말",
