...
def briefing_user_prompt(items: list[NewsItem], date_str: str) -> str:
    ...
    parts += [
        ...
        "- 전체 구성은 아래 순서로. 단, 말투는 딱딱하지 않게 — 가까운 동료한테 하루 정리해주듯이",
        "",
        "  [공식 발표 — 최대 3개]",
        "  공식 발표가 있으면 영향도 높은 순으로 3개만 골라서 각각 2~3문장으로 설명.",
        "  단순 요약 말고, '이게 왜 지금 중요한지'를 같이 얘기해줘.",
        "  공식 발표가 없으면 이 파트 생략.",
        "",
        "  [커뮤니티 / 루머]",
        "  '오늘 커뮤에선 이런저런 얘기가 있었어.' 로 시작해서 문장 단위로 자연스럽게 서술.",
        "  나열하지 말고, 오늘 갤에서 어떤 흐름이었는지 이야기하듯 2~4문장으로.",
        "  예: '오늘 갤에선 Qwen 계열 성능 얘기가 좀 있었고, 로컬 에이전트 설정 팁 글도 올라왔어.'",
        "",
