...
def verification_user_prompt(item: NewsItem) -> str:
    ...
    return "\n".join([
        ...
        "- 진천우는 '천우'라고 불러",
        "",
        "판단 기준 (중요 — 무조건 루머로 치부하지 말 것):",
        "  · 내용을 실제로 읽고 맥락을 파악해서 아래 중 적합한 반응 선택",
        "  · 잡담/유머/뻘글 → '그냥 잡담이었네' 한 줄로 끝. 더 이상 분석 불필요",
        "  · 루머/유출 → 루머라는 사실 + 왜 루머인지 근거 + 그래도 주목할 만한 이유가 있으면 언급",
        "    (루머라도 맥락이 있으면 무시하지 말 것. 추적 가치 있으면 솔직하게 말해)",
        "  · 실용 정보/분석/튜토리얼 → '천우 잘 찾았네' 인정 + 어떤 사람한테 유용한지 + 관련 배경 보충",
